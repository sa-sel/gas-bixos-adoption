...
const displayHelp = () => ui.showModalDialog(HtmlService.createHtmlOutputFromFile('help').setWidth(670).setHeight(870), 'Como usar a planilha?');
  ...
  try {
    bixos.forEach(bixo => {
      ...
      if (!bixo.name || !['Eletrônica', 'Automação'].includes(bixo.emphasis)) {
        throw new Error('Ocorreu um erro com os dados dos bixos. Por favor, verifique se todos os bixos possuem uma ênfase especificada ("Eletrônica" x "Automação").');
      }
    });
  }
  catch (e) {
    // caso algum erro tenha sido acusado ao pegar os dados dos bixos, alerta o usuário e retorna None
    ui.alert('Erro!', 'Ocorreu um erro com os dados dos bixos. Por favor, verifique se todos os bixos possuem uma ênfase especificada ("Eletrônica" x "Automação").', ui.ButtonSet.OK);
    return null;
  }
  return bixos;
};
  ...
  try {
    veteranos.forEach(veterano => {
      ...
      if (!veterano.name || !veterano.email || !['Eletrônica', 'Automação'].includes(veterano.emphasis)) {
        throw new Error('Ocorreu um erro com os dados dos veteranos. Por favor, verifique se todos os veteranos possuem pelo menos nome, email e ênfase especificados ("Eletrônica" x "Automação").');
      }
    });
  }
  catch (e) {
    // caso algum erro tenha sido acusado ao pegar os dados dos veteranos, alerta o usuário e retorna None
    ui.alert('Erro!', 'Ocorreu um erro com os dados dos veteranos. Por favor, verifique se todos os veteranos possuem pelo menos nome, email e ênfase especificados ("Eletrônica" x "Automação").', Button);
    return null;
  }
...
const raffle = () => {
  ...
  const veteranos = getVeteranosDB().filter(veterano => {
    if (veterano.monobixo) {
      ...
      for (let current of occurrences) {
  ...
  if (!veteranos.length) {
    ui.alert('Erro!', 'Há apenas veteranos "monobixo" cadastrados e todos eles possuem um bixo atualmente, portanto não há padrinhos/madrinhas disponíveis.\n\nOBS: os "monobixos" são os veteranos que optaram por receber no máximo um único bixo.', ui.ButtonSet.OK);
    return null;
  }
...
const clearMasterSheet = () => {
  const r = ui.prompt('Tem certeza que deseja continuar?', 'Esse processo irá apagar >>TODOS<< os dados da planilha principal, "Lista dos Meliantes". Não será possível recuperá-los depois disso.', ui.ButtonSet.YES_NO);
...
const getEmailSubject = name => {
  return emailSubject.replace('$(nomeVeteran@)', name) || 'Adoção de Bixos da Elétrica ' + new Date().getFullYear();
};
...
const sendEmails = bixos => {
  if (emailBody.search(/<[u,o]l>[^<uol>]*?\$\(listarBixos\)[^<uol>]?<\/[u,o]l>/gms) < 0) {
    ui.alert('Erro!', 'Infelizmente os emails para os padrinhos/madrinhas não puderam ser enviados - o corpo do email está num formato inválido.\n\nÉ necessário incluir a variável $(listarBixos) no texto e ela deve estar dentro de uma lista.\n\nEx: <ol>$(listarBixos)</ol>, <ul>$(listarBixos)</ol>.\n\nVale lembrar que essa variável será substituída por uma lista de bixos no formado: <li>NOME_DO_BIXO (ÊNFASE_DO_BIXO)</li>.', ui.ButtonSet.OK);
    return;
  }
  else if (Session.getActiveUser().getEmail() != ALLOWED_EMAIL) {
    ui.alert('Erro!', `Infelizmente os emails para os padrinhos/madrinhas não puderam ser enviados. Para enviá-los, é necessário rodar o script com o ${ALLOWED_EMAIL}.`, ui.ButtonSet.OK);
    return;
  }
  ...
  bixos.forEach(bixo => infoGodparents[bixo.godparent]
    ...
    for (let veterano of Object.keys(infoGodparents)) {
...
const createTable = bixos => {
  ...
  let r = ui.prompt('Qual é o título dessa chamada?', 'Ex: "Primeira Chamada FUVEST", "Segunda Chamada SISU", "Terceira Chamada", etc\n', ui.ButtonSet.OK);
  while (!r.getResponseText()) {
    r = ui.prompt('Qual é o título dessa chamada?', 'É necessário inserir o título da chamada para prosseguir.\n\nEx: "Primeira Chamada FUVEST", "Segunda Chamada SISU", "Terceira Chamada", etc', ui.ButtonSet.OK);
  }
  // Título da chamada
...
const main = () => {
  ...
  if (Session.getActiveUser().getEmail() != ALLOWED_EMAIL) {
    const r = ui.prompt('Os emails para os padrinhos/madrinhas não serão enviados!', `Para enviá-los, é necessário rodar o script com o ${ALLOWED_EMAIL}. Você deseja continuar com o sorteio dos bixos mesmo assim?`, ui.ButtonSet.YES_NO);
    if (r.getSelectedButton() == ui.Button.NO) {
      return;
    }
  }
  else if (emailBody.search(/<[u,o]l>[^<uol>]*?\$\(listarBixos\)[^<uol>]?<\/[u,o]l>/gms) < 0) {
    const r = ui.prompt('Os emails para os padrinhos/madrinhas não serão enviados!', 'O corpo do email para os veteranos está num formato inválido. Você deseja continuar com o sorteio dos bixos mesmo assim?\n\nOBS: É necessário incluir a variável $(listarBixos) no texto e ela deve estar dentro de uma lista. Essa variável será substituída por uma lista de bixos no formato: <li>NOME_DO_BIXO (ÊNFASE_DO_BIXO)</li>.\nEx: <ol>$(listarBixos)</ol>, <ul>$(listarBixos)</ol>.', ui.ButtonSet.YES_NO);
    if (r.getSelectedButton() == ui.Button.NO) {
      return;
    }
  }
  else {
    ui.alert('Cuidado!', 'Vou te mostrar um exemplo dos emails que serão enviados para os padrinhos (mas infelizmente você só vai ter uns 20 segundos pra ler ele). Para visualizar o texto, é necessário que você tente "scrollar" no diálogo a seguir.\n\nVocê pode ler o modelo com mais calma na planilha "Template do Email", através do botão "VISUALIZAR EMAIL".', ui.ButtonSet.OK);
